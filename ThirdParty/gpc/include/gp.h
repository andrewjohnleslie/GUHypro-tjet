...
public:
  ...
  GPNode (int nVal, const char* str, int args=0) : nodeValue(nVal),
...
public:
  ...
  virtual int isFunction () const { return node->isFunction (); }
  virtual int isTerminal () const { return node->isTerminal (); }
...
public:
  ...
  virtual char* load (istream& is);
  virtual void save (ostream& os);
  virtual void resolveNodeValues (GPAdfNodeSet& adfNs);
