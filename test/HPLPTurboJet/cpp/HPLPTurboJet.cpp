...
//					std::cout << "Starting New Throttle Position! " << "\n" << "----------------------------------------" << "\n\n";
					...
					if (throttle == throttleDP && Altitude == AltitudeDP && Mach == MachDP)
					{
						...
						std::pair<bool, outputData> DP = HPLPTurboJet (pDP, TDP, MachDP, throttleDP, 1, ondesign, CaptureArea, T04_, T045_, Aratio_HP, Aratio_LP, kH_HP, kH_LP);
...
//		                DP.second.sys->saveJson("/home/mark/Documents/MATLAB/OffDesign/pyCycleComparison/ConcordeSST/DUMPpycycleHPLPHyProRFPDP.txt");
					...
					else
					{
						...
						std::pair<bool, outputData> DP = HPLPTurboJet (pDP, TDP, MachDP, throttleDP, verbosity, ondesign, CaptureArea, T04_, T045_, Aratio_HP, Aratio_LP, kH_HP, kH_LP);
//		                DP.second.sys->saveJson("/home/mark/Documents/MATLAB/OffDesign/pycycleHPLPHyProDP.txt");
						...
						offDP = HPLPTurboJet (p, T, Mach, throttle, verbosity, ondesign, CaptureArea, T04_, T045_, Aratio_HP, Aratio_LP, kH_HP, kH_LP);
...
int main(int argc, char *argv[]) {
	...
	std::vector<double> throttleRange = {0.25};
//	std::vector<double> MachRange = {1.2};
//	std::vector<double> AltitudeRange = {12192};
//	std::vector<double> throttleRange = {0.219};

//	const double MachDP = 2;
//	const double AltitudeDP = 18288;
//	const double throttleDP = 0.1813;
	const double MachDP = 2;
	const double AltitudeDP = 16154;
	const double throttleDP = 0.201;
