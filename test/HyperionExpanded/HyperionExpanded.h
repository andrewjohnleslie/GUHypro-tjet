...
Hyperion(const double &p, const double &T, const double &Mach, const double &throttle, const int mode,
         const int &verbosity) {
/** Modes:
...
//	double A[numNodes] =   {27.0, 27.0*throttle, 0.25*27, 8.24, 11.25, 11.25, 0.0, 11.25-8.24, 22.5, 22.5, 22.5, 95};
    double A[numNodes] =   {27.0, 27.0, 0.25*27, 8.24, 11.25, 11.25, 0.0, 11.25-8.24, 22.5, 22.5, 22.5, 44.3};
//    double A[numNodes] = {46.0, 46.0 * throttle, 3.97, 8.24, 38.3, 38.3, 0.0, 11.25 - 8.24, 38.3, 38.3, 38.3, 184};
    ...
    secondaryC->verbosity_ = verbosity - 1;
...
//		dynamic_pressure = (0.5 * freeStream.rho() * std::pow(freeStream.getU(), 2));
...
//		total_flow = system.N2().rho()*system.N2().getU()*system.N2().A();
        // std::cout << system.N2().report() << std::endl;
//    emissions.clear();
//    emissions.push_back(system.N2().X("H2O"));
//    emissions.push_back(system.N2().X("N2"));
//    emissions.push_back(system.N2().X("O2"));
//    emissions.push_back(system.N2().X("OH"));
//    emissions.push_back(system.N2().X("NO"));
//    emissions.push_back(system.N2().X("NO2"));
...
//			out.push_back(out1[i]);
        ...
        if (verbosity > 0) {
            ...
            //       ss << p;
            //       std::string s(ss.str());
            //       std::ofstream fout("filename" + s + ".txt");
            // selMode.printOut(fout);
