...
#ifndef INJECTIONPHI_H_
#define INJECTIONPHI_H_
...
namespace hypro {
	...
	class InjectionPhi : public Delta {
	...
	public:
		...
		InjectionPhi(std::string name, Node *in, Node *N2);

		InjectionPhi(std::string name, Node *in, Node *N2, int ID);

		InjectionPhi(const InjectionPhi &mod);

		virtual ~InjectionPhi();

		virtual GPObject &duplicate() { return *(new InjectionPhi(*this)); }

		propModuleSERIALtemplate(InjectionPhi);
...
//BOOST_CLASS_EXPORT_KEY(InjectionPhi<balanceMach>);
//BOOST_CLASS_EXPORT_KEY(InjectionPhi<Friction>);
}

#endif /* INJECTIONPHI_H_ */
